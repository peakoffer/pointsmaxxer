...
"""United MileagePlus scraper for PointsMaxxer."""
...
@register_scraper("united")
class UnitedScraper(BaseScraper):
    """Scraper for United MileagePlus awards."""
    ...
    async def search_awards(
        ...
        """Search for United award availability."""
        browser = await self._ensure_browser()
        cache = await self._ensure_cache()

        cache_key = f"united_{origin}_{destination}_{date.strftime('%Y-%m-%d')}_{cabin.value}"
        ...
        async with browser.get_page() as page:
            ...
            try:
                ...
                await page.wait_for_selector(
                    ".flight-result-list, .app-components-Shopping-FlightResultsSection, .no-results",
                    timeout=30000
                )
                ...
                raise ParseError(f"Failed to parse United results: {e}")
    ...
    def _build_search_url(
        ...
        cabin_code = self.CABIN_MAP.get(cabin, "econ")
    ...
    async def _parse_results(
        ...
        """Parse United search results."""
        awards = []

        no_results = await page.query_selector(".no-results, .no-flights-message")
        if no_results:
            return []

        flight_cards = await page.query_selector_all(
            ".flight-result-item, [data-test='flight-card']"
        )
    ...
    async def _parse_flight_card(
        ...
        """Parse a single United flight card."""
        try:
            flight_no_elem = await card.query_selector(
                ".flight-number, [data-test='flight-number']"
            )
            flight_no = await flight_no_elem.inner_text() if flight_no_elem else "UA???"

            dep_time_elem = await card.query_selector(
                ".departure-time, [data-test='departure-time']"
            )
            arr_time_elem = await card.query_selector(
            ...
            miles_elem = await card.query_selector(
            ...
            fees_elem = await card.query_selector(
            ...
            is_saver = False
            saver_elem = await card.query_selector(
            ...
            operating_elem = await card.query_selector(".operated-by, .operating-carrier")
            airline_name = "United Airlines"
            airline_code = "UA"
            if operating_elem:
                operating_text = await operating_elem.inner_text()
                if "Lufthansa" in operating_text:
                    airline_name = "Lufthansa"
                    airline_code = "LH"
                elif "ANA" in operating_text:
                    airline_name = "ANA"
                    airline_code = "NH"
                elif "Swiss" in operating_text:
                    airline_name = "Swiss"
                    airline_code = "LX"
